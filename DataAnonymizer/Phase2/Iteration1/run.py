...
class input_processor_class:
    def __init__(self):
        ...
        with open(join(input_folder, "variables.dictionary")) as f:
            ...
            self.columns_to_encrypt = data["columns_to_encrypt"]
            self.columns_to_decrypt = data["columns_to_decrypt"]
...
class encrypter:
    def __init__(self, columns_to_encrypt):
        self.columns = {}
        self.process_string(columns_to_encrypt, self.columns)
    ...
    def process_string(self, s, dictionary):
        ...
        for entry in entries:
            ...
            if len(parts) == 2:
                col = parts[0]
                key = parts[1]
                key = self.password_to_key(key)
                dictionary[col] = key

    def encrypt_df(self, df):
        for col_name, encrypt_key in self.columns.items():
            if col_name in df.columns:
                for i, cell in enumerate(df[col_name]):
                    encrypted_value = self._encrypt(cell, encrypt_key).decode('utf-8')
                    df.at[i, col_name] = encrypted_value

    def _encrypt(self, s, key):
        fernet = Fernet(key)
        return fernet.encrypt(s.encode('utf-8'))

    def password_to_key(self, password):
        password = password.encode('utf-8')
        salt = b''  # Use a constant salt for consistency
        kdf = PBKDF2HMAC(
...
class decrypter:
    def __init__(self, columns_to_decrypt):
        self.columns = {}
        self.process_string(columns_to_decrypt, self.columns)
    ...
    def decrypt_df(self, df):
        for col_name, decrypt_key in self.columns.items():
            if col_name in df.columns:
                for i, cell in enumerate(df[col_name]):
                    decrypted_value = self._decrypt(cell, decrypt_key)
                    df.at[i, col_name] = decrypted_value

    def _decrypt(self, s, key):
        fernet = Fernet(key)
        try:
            dec_message = fernet.decrypt(s.encode('utf-8')).decode('utf-8')
        except:
            dec_message = "incorrect key"
        return dec_message
...
if __name__ == "__main__":
    ...
    encrypter = encrypter(input_processor.columns_to_encrypt)
    encrypter.encrypt_df(df)

    decrypter = decrypter(input_processor.columns_to_decrypt)
    decrypter.decrypt_df(df)
